...
def fetch_recent_plays_for_polling(
    user_record: Any,
    limit: int = MAX_RECENTLY_PLAYED,
) -> list[tuple[str, datetime]]:
    """Return `[(spotify_track_id, played_at_utc), ...]` for the user.

    Public wrapper around `GET /me/player/recently-played` designed for
    the P5 T51 sandbox-play polling loop in `routes/jobs.py`. Unlike
    `_real_recently_played` (which returns full track dicts for a
    snapshot), this returns just the `(track_id, played_at)` pairs the
    polling loop needs, with `played_at` parsed as a UTC-aware
    `datetime` and rounded to the nearest second so dedup keys stay
    stable across retries.

    Mock mode: always returns `[]`. Callers that hit this in mock
    mode are almost certainly a test that forgot to flip
    `settings.mock_mode` - not a production request - and returning
    an empty list is the least surprising thing.

    Raises SpotifyAuthError on 401 (bubbled from `_ensure_fresh_token`).
    All other failures are the caller's problem; log + skip in T51.
    """
    ...
    with _spotify_client(access_token) as client:
        data = _get_with_retry(client, "/me/player/recently-played", params={
            "limit": min(int(limit), MAX_RECENTLY_PLAYED),
        })
    ...
    for item in (data.get("items") or []):
        ...
        if not track_id or not played_at_str:
            continue
        try:
            # Spotify returns ISO-8601 with a trailing 'Z'; Python's
            # fromisoformat handles the tz-aware suffix natively in 3.11+.
            played_at = datetime.fromisoformat(played_at_str.replace("Z", "+00:00"))
        except ValueError:
            continue
        # Normalise to a UTC-naive datetime to match how the rest of
        # the app stores timestamps (via `datetime.utcnow()`), which
        # keeps ORM equality checks predictable.
        played_at_utc = played_at.astimezone(timezone.utc).replace(
            tzinfo=None, microsecond=0,
        )
        out.append((track_id, played_at_utc))
    return out
