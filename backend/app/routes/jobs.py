...
from app.spotify_client import (
    SpotifyAuthError,
    fetch_recent_plays_for_polling,
...
USERS_FIXTURE_PATH: Path = settings.mock_data_dir / "mock_users.json"
...
def _load_mock_users() -> dict[str, dict[str, Any]]:
    """Return `{user_id: user_row}` from mock_users.json.

    Empty dict if the file is missing (older fixtures). Callers must
    default to `eligible_for_pulse=True` when a user has no row here,
    so pre-Riya fixtures keep firing nudges for Aanya + Karthik.
    """
    if not USERS_FIXTURE_PATH.exists():
        return {}
    data = json.loads(USERS_FIXTURE_PATH.read_text(encoding="utf-8"))
    return {u["id"]: u for u in data.get("users", [])}


def _eligibility_reason(user_row: dict[str, Any] | None) -> tuple[bool, str]:
    """Decide whether Pulse should run detection for this user.

    Anchored to `mock_users.json` — the same source-of-truth the
    frontend reads via `GET /users`. Returns
    `(eligible, human_readable_reason)` so the JobRun trace shows
    exactly *why* a user was skipped, not just that they were.
    """
    if user_row is None:
        return True, "no roster row (defaulting eligible)"
    if user_row.get("eligible_for_pulse") is False:
        why = user_row.get("why_ineligible") or "eligible_for_pulse=false"
        return False, f"not_eligible: {why}"
...
def _wipe_user_history(db, user_id: str) -> None:
    ...
    tracks and any sandbox play events). This is mock-mode demo
    behaviour: every detection run is designed to be a fresh start
    so a presenter can re-run the loop repeatedly without state from
    prior runs blocking the trigger (e.g. a half-finished reset
    session marking `has_active_session=True` and silently
    suppressing nudges).
    ...
    if user_session_ids:
        # T26 (P5): play events reference reset_sessions via FK, so
        # they must be cleared before the parent rows to keep bulk
        # SQL DELETEs (which skip ORM cascade) FK-safe.
        db.query(SandboxPlayEvent).filter(
            SandboxPlayEvent.reset_session_id.in_(user_session_ids)
...
# ============================================================
# T51 (P5) - sandbox play polling
# ============================================================
#
# `_poll_sandbox_plays` runs at the tail of every real-mode
# `/jobs/run-detection` call. For each user with an active
# `ResetSession`, we hit `GET /me/player/recently-played` and
# insert one `SandboxPlayEvent` row per unique (track, played_at)
# where the played track belongs to the session's track list.
#
# Design:
#   - Mock mode is a no-op (no writes, no fetches).
#   - Any Spotify auth or transport failure is caught + logged; a
#     poll failure MUST NOT abort the surrounding detection run.
#   - Idempotency: `(reset_session_id, spotify_track_id, played_at)`
#     is the dedup key. Spotify sometimes returns the same play
#     twice across polls, so we check-then-insert.
#   - Bounded work per user: recently-played caps at 50 items on
#     Spotify's side; we walk that list once.

def _poll_sandbox_plays(db, active_sessions: list[ResetSession]) -> dict[str, Any]:
    """Poll recently-played for each active session, insert play events.

    Returns a small trace dict summarising the work done so the
    caller can attach it to the JobRun details for the run-history
    view. Real mode only; mock mode returns a `skipped` trace.
    """
    if settings.mock_mode:
        return {"skipped": "mock_mode", "sessions": 0, "events_inserted": 0}
    if not active_sessions:
        return {"skipped": "no_active_sessions", "sessions": 0, "events_inserted": 0}

    trace: dict[str, Any] = {"sessions": len(active_sessions), "events_inserted": 0,
                             "per_session": []}
    for session in active_sessions:
        user = db.get(User, session.user_id)
        if user is None or not user.access_token:
            trace["per_session"].append({
                "session_id": session.id, "user_id": session.user_id,
                "skipped": "no_access_token",
            })
            continue

        try:
            plays = fetch_recent_plays_for_polling(user_record=user)
        except SpotifyAuthError as exc:
            log.warning(
                "poll_sandbox_plays auth failed | session=%s user=%s err=%s",
                session.id, session.user_id, exc,
            )
            trace["per_session"].append({
                "session_id": session.id, "user_id": session.user_id,
                "error": f"auth: {exc}",
            ...
            # Non-fatal: log and move to the next session so one bad
            # user does not block the polling loop for the rest.
            log.warning(
                "poll_sandbox_plays fetch failed | session=%s user=%s err=%s",
                session.id, session.user_id, exc,
            )
            trace["per_session"].append({
                "session_id": session.id, "user_id": session.user_id,
        ...
        session_track_ids = {t.spotify_track_id for t in session.tracks}
        inserted_here = 0
        for track_id, played_at in plays:
            if track_id not in session_track_ids:
                continue
            already = (
                db.query(SandboxPlayEvent)
                .filter(
                    SandboxPlayEvent.reset_session_id == session.id,
                    SandboxPlayEvent.spotify_track_id == track_id,
                    SandboxPlayEvent.played_at == played_at,
                )
                .first()
            )
            if already is not None:
                continue
            db.add(SandboxPlayEvent(
                id=str(uuid.uuid4()),
        ...
        trace["events_inserted"] += inserted_here
        trace["per_session"].append({
            "session_id": session.id, "user_id": session.user_id,
...
def run_weekly_detection(
    ...
    with db_session() as db:
        for user_id in user_ids:
            ...
            user_row = mock_users.get(user_id)
            is_eligible, eligibility_reason = _eligibility_reason(user_row)

            if not is_eligible:
                # Pulse structurally does not offer nudges to users outside
                # the target segment (Premium, 3+ years tenure, 5+ days/week,
                # self-aware, wants discovery). We still upsert the User row
                # + wipe stale history for idempotency, but we skip
                # detection entirely and record the reason in the trace.
                if not dry_run:
                ...
                summary["details"].append({
                    ...
                    "latest_overall":         None,
                    "latest_suggested_scope": None,
                    "nudge_id":               None,
                    "skipped_by_eligibility": True,
                    "plan":                   (user_row or {}).get("plan"),
                    "tenure_months":          (user_row or {}).get("tenure_months"),
                })
                continue
...
def _run_real_mode(*, dry_run: bool, allow_empty: bool = False) -> dict[str, Any]:
    ...
    with db_session() as db:
        ...
        # T51 (P5): after the per-user snapshot pass, poll recently-
        # played for every active sandbox and log the plays. This
        # is best-effort: any failure here MUST NOT taint the
        # snapshot / score / nudge writes above.
        if not dry_run:
            try:
                active = (
                    db.query(ResetSession)
                    .filter(ResetSession.decision.is_(None))
                    .all()
                )
                poll_trace = _poll_sandbox_plays(db, active)
                summary["sandbox_poll"] = poll_trace
            except Exception as exc:                                        # noqa: BLE001
                log.warning("sandbox play polling failed (non-fatal): %s", exc)
                summary["sandbox_poll"] = {"error": str(exc)}
            db.commit()
